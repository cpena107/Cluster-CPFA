...
void Minimap_controller::Returning() {
	...
	if(IsInTheNest() == true) {
		...
		// If returning without food due to resource loss, keep visited locations for next search
			...
			// Mark that we lost the resource, will search unvisited areas next time
			isLostResource = true;
		}
...
void Minimap_controller::SetUnvisitedSearchLocation() {
	...
	// Try to find an unvisited location
	while(attempts < MAX_ATTEMPTS && !foundUnvisited) {
		argos::Real random_wall = RNG->Uniform(argos::CRange<argos::Real>(0.0, 1.0));
	...
	// If we found an unvisited location, use it. Otherwise, use the last candidate.
	SetIsHeadingToNest(false);
	SetTarget(candidate);
...
/*****
 * Share the visited locations with the nest (simulated as storing in loop functions).
